...
class Gait3D(OpenSimGym):
    ...
    def get_effort(self):
        outputs = self.model.get_outputs()
        muscle_activity = np.sum(np.power(outputs['muscle_activations'], 2))
        return self.weights[0] * muscle_activity
    ...
    def get_pain(self):
        ...
        pain = self.weights[1] * limit_torques
        if (grfs[1] / self.body_weight) > self.grf_threshold:
            pain += self.weights[2] * grfs[1]
...
class Gait3DAdaptation(OpenSimGym):
